...
class CookieStore {
  config:
    | ((options: { requestAlias: PropertyKey; uploadFileAlias: PropertyKey; downloadFileAlias: PropertyKey }) => void)
    | undefined;
   ...
   * @return {Boolean}            是否存在
   */
  has(name?: string, domain?: string, path?: string) {
    // 返回是否存在 cookie 值
   ...
   */
  set(name = '', value = '', options: { domain?: string } = {}) {
    // 构建 Cookie 实例
    ...
    // 设置到指定域名
    const cookies = this.__cookiesMap.get(domain) || new Map();
   ...
   */
  getRequestCookies(domain: string | undefined, path: string | undefined) {
    // cookies 数组
   ...
   */
  setResponseCookies(setCookieStr: string | undefined, domain: string) {
    // 转换为 cookie 对象数组
   ...
   * @return {Array}               Cookie 对象数组
   */
  parse(setCookieStr = '', domain: any) {
    // parse
    const cookies = cookieParser.parse(cookieParser.splitCookiesString(setCookieStr), { decodeValues: false });
  ...
  stringify(cookies: any[]) {
    return cookies.map((item: { toString: () => any }) => item.toString()).join('; ');
  }
