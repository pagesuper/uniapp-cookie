...
var CookieStore = /** @class */ (function () {
    ...
    CookieStore.prototype.dir = function () {
        var dirObj = {};
        for (var _i = 0, _a = this.__cookiesMap.keys(); _i < _a.length; _i++) {
            var domain = _a[_i];
    ...
    CookieStore.prototype.remove = function (name, domain) {
        ...
        if (domain) {
            ...
            // 删除所有域名的 cookie
            for (var _i = 0, _a = this.__cookiesMap.values(); _i < _a.length; _i++) {
                var cookies = _a[_i];
                cookies.delete(name);
            }
        }
    ...
    CookieStore.prototype.getCookie = function (name, domain, path) {
        ...
        // 获取任意域名的 cookie
        for (var _i = 0, _a = this.__cookiesMap.entries(); _i < _a.length; _i++) {
    ...
    CookieStore.prototype.getCookiesArray = function (domain, path) {
        ...
        for (var _i = 0, _a = this.__cookiesMap.entries(); _i < _a.length; _i++) {
            ...
            if (domain && scopeDomains.indexOf(key) < 0)
                continue;
            // 循环当前域名下所有 cookie
            for (var _c = 0, _d = cookies.values(); _c < _d.length; _c++) {
                var cookie = _d[_c];
    ...
    CookieStore.prototype.__saveToStorage = function () {
        try {
            ...
            for (var _i = 0, _a = this.__cookiesMap.values(); _i < _a.length; _i++) {
                var cookies = _a[_i];
                for (var _b = 0, _c = cookies.values(); _b < _c.length; _b++) {
                    var cookie = _c[_b];
                    if (cookie.isExpired()) {
                        // 清除无效 cookie
